...
public class AttackBean implements Serializable {
    ...
    private Pair<String, SmtNode> prefix;
    private Pair<String, SmtNode> suffix;
    private Pair<String, SmtNode> infix;
    ...
    public Pair<String, SmtNode> getPrefix() {
        return prefix;
    }

    public void setPrefix(Pair<String, SmtNode> prefix) {
        this.prefix = new Pair<>(prefix.getKey().replace("\\f", "\f")
                .replace("\\n", "\n")
                .replace("\\t", "\t")
                .replace("\\r", "\r")
                .replace("\\v", "\u000b")
    ...
    public Pair<String, SmtNode> getSuffix() {
        return suffix;
    }

    public void setSuffix(Pair<String, SmtNode> suffix) {
        this.suffix = new Pair<String, SmtNode>(suffix.getKey().replace("\\f", "\f")
                .replace("\\n", "\n")
                .replace("\\t", "\t")
                .replace("\\r", "\r")
                .replace("\\v", "\u000b")
    ...
    public Pair<String, SmtNode> getInfix() {
        return infix;
    }

    public void setInfix(Pair<String, SmtNode> infix) {
        this.infix = new Pair<>(infix.getKey().replace("\\f", "\f")
                .replace("\\n", "\n")
                .replace("\\t", "\t")
                .replace("\\r", "\r")
                .replace("\\v", "\u000b")
...
//    }
    ...
    public String getAttackString() {
        StringBuilder stringBuilder = new StringBuilder(prefix.getKey());
        for (int i = 0; i < repeatTimes; i++) {
            stringBuilder.append(infix.getKey());
        }
        stringBuilder.append(suffix.getKey());
        return stringBuilder.toString();
    }

    public String getAttackStringFormatType() {
        String formatPrefix = getPrefix().getKey().replace("\\", "\\\\")
                .replace("\f", "\\f")
                .replace("\n", "\\n")
                .replace("\t", "\\t")
                .replace("\r", "\\r")
                .replace("\u000b", "\\u000b");
        String formatRepeat = getInfix().getKey().replace("\\", "\\\\")
                .replace("\f", "\\f")
                .replace("\n", "\\n")
                .replace("\t", "\\t")
                .replace("\r", "\\r")
                .replace("\u000b", "\\u000b");;
        String formatSuffix = getSuffix().getKey().replace("\\", "\\\\")
                .replace("\f", "\\f")
                .replace("\n", "\\n")
                .replace("\t", "\\t")
                .replace("\r", "\\r")
    ...
    public boolean equals(Object o) {
        ...
        if (!TrimUtils.rtrim(prefix.getKey(), infix.getKey()).equals(TrimUtils.rtrim(bean.prefix.getKey(), bean.infix.getKey()))) {
            return false;
        }
        if (!suffix.getKey().equals(bean.suffix.getKey())) {
            return false;
        }
        return infix.getKey().equals(bean.infix.getKey());
    }

    @Override
    public int hashCode() {
        int result = TrimUtils.rtrim(prefix.getKey(), infix.getKey()).hashCode();
        result = 31 * result + suffix.getKey().hashCode();
        result = 31 * result + infix.getKey().hashCode();
        return result;
    }
