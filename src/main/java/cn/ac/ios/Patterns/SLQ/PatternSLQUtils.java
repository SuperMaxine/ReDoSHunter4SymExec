...
//        ReDoSTree = removeBlankStr(ReDoSTree);
    ...
    private static ArrayList<AttackBean> getSLQConditionOne(TreeNode root, String regex) throws InterruptedException {
        ...
        for (Pair<String, String> pair : pairArrayList) {
            ...
            Pair<String, SmtNode> suffix = new Pair<>(pair.getValue(), new SmtNode(Collections.singletonList(pair.getValue())));
            AttackBean attackBean = new AttackBean();
            attackBean.setPrefix(new Pair<String, SmtNode>("", new SmtNode(Collections.singletonList(""))));
            attackBean.setInfix(new Pair<>(repeat, new SmtNode(Collections.singletonList(repeat))));
...
//            node = node.getNextNode();
    ...
    private static ArrayList<AttackBean> getSLQConditionTwo(TreeNode ReDoSTree, String regex) throws InterruptedException {
        ...
        for (int i = 0; i < treeNodeList.size(); i++) {
            if (treeNodeList.get(i).getData().length() >= 3) {
                ...
                for (AttackBean attackBean : sublist) {
                    ...
                    originAttackBean.setPrefix(new Pair<>(originAttackBean.getPrefix().getKey() + stringBuilder.toString(), new SmtNode(originAttackBean.getPrefix().getValue())));
                    if (i > 1) {
                        String tmp = treeNodeList.get(i - 1).getNonMatchStr();
                        originAttackBean.setSuffix(new Pair<>(tmp, new SmtNode(Collections.singletonList(tmp))));
                    }
                    list.add(originAttackBean);

                    attackBean.setPrefix(new Pair<>(attackBean.getPrefix().getKey() + stringBuilder.toString(), new SmtNode(attackBean.getPrefix().getValue())));
                    if (i > 1) {
                        String tmp = treeNodeList.get(i - 1).getNonMatchStr() + attackBean.getSuffix().getKey();
                        attackBean.setSuffix(new Pair<>(tmp, new SmtNode(Collections.singletonList(tmp))));
                    }
                }
                list.addAll(sublist);
            }
        ...
        for (Integer index : dotList) {
            ...
            AttackBean attackBean = new AttackBean();
            attackBean.setPrefix(new Pair<String, SmtNode>("", new SmtNode(Collections.singletonList(""))));
            attackBean.setInfix(new Pair<>(repeat.toString(), new SmtNode(Collections.singletonList(repeat.toString()))));
            attackBean.setSuffix(new Pair<>(suffix.toString(), new SmtNode(Collections.singletonList(suffix.toString()))));
            attackBean.initType(AttackType.POLYNOMIAL);
            ...
            if (suffix.toString().contains(" ")) {
                ...
                attackBean.setPrefix(new Pair<String, SmtNode>("", new SmtNode(Collections.singletonList(""))));
                attackBean.setInfix(new Pair<>(repeat.toString().replace(" ", ""), new SmtNode(Collections.singletonList(repeat.toString().replace(" ", "")))));
                attackBean.setSuffix(new Pair<>(suffix.toString().replace(" ", ""), new SmtNode(Collections.singletonList(suffix.toString().replace(" ", "")))));
                attackBean.initType(AttackType.POLYNOMIAL);
                attackBean.setPatternType(PatternType.SLQ_2);
                list.add(attackBean);
            }
        ...
        while (!stack.isEmpty()) {
            ...
            if (isSLQThreeCountingNode(node)) {
                ...
                for (String str : arrayList) {
                    AttackBean attackBean = new AttackBean();
                    String tmp = root.getNonMatchStr(node);
                    attackBean.setPrefix(new Pair<>(tmp, new SmtNode(Collections.singletonList(tmp))));
                    String tmpSuffix = root.getNonMatchStr(node) + root.getNonMatchStr();
                    attackBean.setSuffix(new Pair<>(tmpSuffix, new SmtNode(Collections.singletonList(tmpSuffix))));
                    attackBean.setInfix(new Pair<>(str, new SmtNode(Collections.singletonList(str))));
                    attackBean.initType(AttackType.POLYNOMIAL);
        ...
        if (regex.startsWith("^") && regex.endsWith("$") && isOrNode(root)) {
            ...
            if (root.getChild(0).getData().equals(root.getChild(2).getData())) {
                ...
                for (AttackBean attackBean : list) {
                    String tmp = root.getChild(1).getNonMatchStr();
                    attackBean.setPrefix(new Pair<>(tmp, new SmtNode(Collections.singletonList(tmp))));
                }
            }
        }
...
//            deleteZeroWidthAssertion(regexTree);
            ...
            if (isOrNode(regexTree)) {
                Pair<StringBuilder,SmtNode> prefixStringBuilder = new Pair<>(new StringBuilder(), new SmtNode(Collections.singletonList("")));
                for (int i = 0; i < regexTree.getChildCount(); i += 2) {
                    Pair<String, SmtNode> prefix = getNoMatchString(regexTree.getChild(i).getData());
                    if (prefix.getKey() != null) {
//                        prefixStringBuilder.append(prefix);
                        prefixStringBuilder.getKey().append(prefix.getKey());
                        prefixStringBuilder.setValue(prefix.getValue().appendNode(prefixStringBuilder.getValue()));
                    }
                }
                ...
                for (int i = 0; i < regexTree.getChildCount() - 2; i += 2) {
                    for (int j = i + 2; j < regexTree.getChildCount(); j++) {
                        ...
                        Pair<String, SmtNode> infix = getExampleByDkBricsAutomaton(regexList1, null, 1);
                        if (infix.getKey() == null) continue;
                        AttackBean attackBean = new AttackBean();
                        attackBean.setPrefix(new Pair<String, SmtNode>(prefixStringBuilder.getKey().toString(), prefixStringBuilder.getValue()));
                        attackBean.setInfix(infix);
                        String tmpSuffix = getSpecialSuffixStringWithCountingAndEndLine(regexTree);
                        attackBean.setSuffix(new Pair<>(tmpSuffix, new SmtNode(Collections.singletonList(tmpSuffix))));
                        attackBean.initType(AttackType.POLYNOMIAL);
                        attackBean.setPatternType(PatternType.SLQ_1);
                        list.add(attackBean);
    ...
    private static ReDoSBean getSLQReDoSBeanHelper(TreeNode sourceReDoSTree, TreeNode standardizedReDoSTree, String regex) throws InterruptedException {
        ...
        for (AttackBean attackBean : list2) {
            ...
            item.setPrefix(new Pair<String, SmtNode>("", new SmtNode(Collections.singletonList(""))));
            item.setInfix(new Pair<>(attackBean.getPrefix().getKey() + attackBean.getInfix().getKey(),
                    new SmtNode(attackBean.getPrefix().getValue().getIntersectionRegexes(),
                            new SmtNode(attackBean.getInfix().getValue().getIntersectionRegexes())
                    )
            ));
            String tmpSuffix = attackBean.getSuffix().getKey();
            item.setSuffix(new Pair<>(tmpSuffix, new SmtNode(Collections.singletonList(tmpSuffix))));
...
//        ArrayList<AttackBean> list4 = getSLQConditionSp(standardizedReDoSTree, regex);
        ...
        list1.addAll(list4);
        list1.sort(Comparator.comparingInt(o -> o.getInfix().getKey().length()));
//        list1.addAll(0, getSLQReDoSBeanHelper2(standardizedReDoSTree, regex).getAttackBeanList());
        ...
        list5.sort(Comparator.comparingInt(o -> o.getInfix().getKey().length()));
    ...
    private static ArrayList<AttackBean> getSLQCondition(TreeNode root, String regex) throws InterruptedException {
        ...
        for (TreeNode node : nodes) {
            ...
            try {
                Pair<String, SmtNode> attackInfix;
                ...
                if (attackInfix == null || attackInfix.getKey().length() == 0) {
                    continue;
                }
                Pair<String, SmtNode> attackSuffix = getNoMatchString(suffixRegex);
                if (attackSuffix.getKey() == null || attackSuffix.getKey().length() == 0) {
                ...
                AttackBean attackBean = new AttackBean();
                attackBean.setPrefix(new Pair<>("", new SmtNode(Collections.singletonList(""))));
    ...
    private static Pair<String, SmtNode> getMatchString(String infixRegex, String suffixRegex) {
    ...
    private static Pair<String, SmtNode> getMatchString(String preRegex, String infixRegex, String suffixRegex) {
        try {
            Pair<String, SmtNode> attackInfix;
            if (suffixRegex.length() != 0) {
                String regex = "(" + preRegex + ".*)＆" + "(" + infixRegex + ")＆(～(" + suffixRegex + "))";
                attackInfix = getMatchString(regex);
                if (attackInfix.getKey() == null || attackInfix.getKey().length() == 0) {
...
//        deleteZeroWidthAssertion(regexTree);
    ...
    private static Pair<String, SmtNode> getMatchString(String regex) throws InterruptedException {
    ...
    private static Pair<String, SmtNode> getNoMatchString(String regex) throws InterruptedException {
        TreeNode node = createReDoSTree("～(" + regex + ")");
        List<String> innerTrans = getTranslateRegexForAssertionsList(node);
        List<String> regexList1 = new ArrayList<>(innerTrans);
        return getExampleByDkBricsAutomaton2(regexList1, 1);
