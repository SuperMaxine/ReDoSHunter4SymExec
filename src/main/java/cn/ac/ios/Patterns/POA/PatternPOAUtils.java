...
//        deleteZeroWidthAssertion(regexTree);
    ...
    private static Set<Pair<String, SmtNode>> getInfixForPOA(String regex1, String regex2, String regex3, boolean considerZeroWidthAssertions) throws InterruptedException {
        if (considerZeroWidthAssertions) {
            ...
            Pair<String, SmtNode> example = getExampleByDkBricsAutomaton2(regexList1, 0);
            if (example.getKey() != null && !example.equals("")) infixSet.add(example);

            regexList1 = new ArrayList<>();
            regexList1.addAll(regex1Regex3Trans);
            regexList1.addAll(regex1Regex2Regex3Trans);
            example = getExampleByDkBricsAutomaton2(regexList1, 0);
            if (example.getValue() != null && !example.equals("")) infixSet.add(example);
            ...
            Pair<String, SmtNode> example = getExampleByDkBricsAutomaton(regexList1);
            if (example.getKey() != null && !example.equals("")) infixSet.add(example);
...
//                        infixSet.addAll(generateInfixStringForPOA(beta1StringBuilder.toString(), r2, child2.getData()));
                        Set<Pair<String, SmtNode>> infixSet1 = getInfixForPOA(beta1StringBuilder.toString(), r2, child2.getData(), false);
                        Set<Pair<String, SmtNode>> infixSet2 = getInfixForPOA(beta1StringBuilder.toString(), r2, child2.getData(), true);
                        if (!infixSet1.isEmpty()) infixSet.addAll(infixSet1);
                        if (!infixSet2.isEmpty()) infixSet.addAll(infixSet2);

                        if (!infixSet.isEmpty()) {

                            Pair<String, SmtNode> prefix1 = getPrefixString(standardizedReDoSTree, child1, true); // 获取的是前缀正则+中缀正则的生成串
                            Pair<String, SmtNode> prefix2 = getPrefixString(standardizedReDoSTree, child1, false);
                            String prefix3string = treeNodeCopy.getMatchStr(treeNodeCopy.getChildByChainIndexString(child1.getChainIndex()));
...
//                            suffixSet.addAll(specialSuffixStringWithNegativeAssertNode);
                            for (String specialSuffix : specialSuffixStringWithNegativeAssertNode) {
                                suffixSet.add(new Pair<>(specialSuffix, new SmtNode(Collections.singletonList(specialSuffix))));
                            }
...
//                            if (specialSuffix2 != null) suffixSet.add(specialSuffix2);
                            for (Pair<String, SmtNode> infix : infixSet) {
                                suffixSet.addAll(getSuffixString(standardizedReDoSTree, child2, infix.getKey()));
                            }
...
//                                infixSet = generateInfixStringForPOA(beta1StringBuilder.toString(), r2, child2.getData());
                                ...
                                if (!infixSet.isEmpty()) {
                                    ...
                                    Pair<String, SmtNode> prefix1 = getPrefixString(r0Tree, childList.get(k), true); // 获取的是前缀正则+中缀正则的生成串
                                    Pair<String, SmtNode> prefix2 = getPrefixString(r0Tree, childList.get(k), false);
