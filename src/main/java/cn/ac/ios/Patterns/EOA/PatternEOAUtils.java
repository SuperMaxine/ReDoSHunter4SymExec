...
//        regex2 = repairLineFeedsCarriageReturnsAndTabCharacters(regex2);
        ...
        String str = getExampleByDkBricsAutomaton(regexList1, regexList2, 0).getKey();
    ...
    private static Set<Pair<String, SmtNode>> getInfixSetForEOA(String r0, String r1, String r2, String r3, String r4, List<Integer> candidateCountingList, boolean considerZeroWidthAssertions) throws InterruptedException {
        if (considerZeroWidthAssertions) {  // 考虑零宽断言
            Set<Pair<String, SmtNode>> infixSet = new HashSet<>();
            ...
            String infix1Str = r0Tree.getMatchStr();
            String infix2Str = getMatchStringWithEndLineInOrNode(r1AndR3Tree);
            String infix3Str = r2Tree.getMatchStr();
            String infix4Str = r4Tree.getMatchStr();
            String tmpStr = infix1Str + infix2Str + infix3Str + infix4Str;
            if (!"".equals(tmpStr)) {
                infixSet.add(new Pair<>(tmpStr, new SmtNode(Collections.singletonList(tmpStr))));
            }


            Pair<String, SmtNode> infix1 = getExampleByDkBricsAutomaton2(r0Trans, 0);
            if (infix1.getKey() == null) return null;

            List<String> r1AndR3Trans = new ArrayList<>();
            r1AndR3Trans.addAll(r1Trans);
            r1AndR3Trans.addAll(r3Trans);

            Set<Pair<String, SmtNode>> infix2Set = new HashSet<>();
            for (int counting: candidateCountingList) {
                Pair<String, SmtNode> infix2 = getExampleByDkBricsAutomaton2(r1AndR3Trans, counting);
                if (infix2.getKey() != null) infix2Set.add(infix2);
            }
            if (infix2Set.isEmpty()) return null;

            Pair<String, SmtNode> infix3 = getExampleByDkBricsAutomaton2(r2Trans, 0);
            if (infix3.getKey() == null) return null;

            Pair<String, SmtNode> infix4 = getExampleByDkBricsAutomaton2(r4Trans, 0);

            for (Pair<String, SmtNode> infix2s: infix2Set) {
                if (!"".equals(infix1.getKey() + infix2s.getKey() + infix3.getKey() + infix4.getKey())) {
                    infixSet.add(new Pair<>(infix1.getKey() + infix2s.getKey() + infix3.getKey() + infix4.getKey(),
                            new SmtNode(infix1.getValue().getIntersectionRegexes(),
                                    new SmtNode(infix2s.getValue().getIntersectionRegexes(),
                                            new SmtNode(infix3.getValue().getIntersectionRegexes(), infix4.getValue())
                                    )
                            )
                    ));
                }
            ...
            Pair<String, SmtNode> infix1 = getExampleByDkBricsAutomaton(new ArrayList<>(Collections.singleton(r0)), null, 0);
            if (infix1.getKey() == null) return null;


            Set<Pair<String, SmtNode>> infix2Set = new HashSet<>();
            for (int counting: candidateCountingList) {
                Pair<String, SmtNode> infix2 = getExampleByDkBricsAutomaton(new ArrayList<>(Arrays.asList(r1, r3)), null, counting);
                if (infix2.getKey() != null) infix2Set.add(infix2);
            }
            if (infix2Set.isEmpty()) return null;

            Pair<String, SmtNode> infix3 = getExampleByDkBricsAutomaton(new ArrayList<>(Collections.singleton(r2)), null, 0);
            if (infix3.getKey() == null) return null;

            Pair<String, SmtNode> infix4 = getExampleByDkBricsAutomaton(new ArrayList<>(Collections.singleton(r4)), null, 0);

            Set<Pair<String, SmtNode>> infixSet = new HashSet<>();
            for (Pair<String, SmtNode> infix2: infix2Set) {
                if (!"".equals(infix1.getKey() + infix2.getKey() + infix3.getKey() + infix4.getKey())) {
                    infixSet.add(new Pair<>(infix1.getKey() + infix2.getKey() + infix3.getKey() + infix4.getKey(),
                            new SmtNode(infix1.getValue().getIntersectionRegexes(),
                                    new SmtNode(infix2.getValue().getIntersectionRegexes(),
                                            new SmtNode(infix3.getValue().getIntersectionRegexes(), infix4.getValue())
                                    )
                            )
                    ));
...
//            List<String> r3Trans = getTranslateRegexForAssertionsList(r3Tree);
//            List<String> r4Trans = getTranslateRegexForAssertionsList(r4Tree);
//            String infix1 = getExampleByDkBricsAutomaton2(r0Trans, 0).getKey();
//            if (infix1 == null) return null;
//
//            List<String> r1AndR3Trans = new ArrayList<>();
//            r1AndR3Trans.addAll(r1Trans);
//            r1AndR3Trans.addAll(r3Trans);
//            String infix2 = getExampleByDkBricsAutomaton2(r1AndR3Trans, counting).getKey();
//            if (infix2 == null) return null;
//
//            String infix3 = getExampleByDkBricsAutomaton2(r2Trans, 0).getKey();
//            if (infix3 == null) return null;
//
//            String infix4 = getExampleByDkBricsAutomaton2(r4Trans, 0).getKey();
//
...
//            r4 = getRegexWithoutZeroWidthAssertion(r4);
//
//            String infix1 = getExampleByDkBricsAutomaton(new ArrayList<>(Collections.singleton(r0)), null, 0).getKey();
//            if (infix1 == null) return null;
//
//            String infix2 = getExampleByDkBricsAutomaton(new ArrayList<>(Arrays.asList(r1, r3)), null, counting).getKey();
//            if (infix2 == null) return null;
//
//            String infix3 = getExampleByDkBricsAutomaton(new ArrayList<>(Collections.singleton(r2)), null, 0).getKey();
//            if (infix3 == null) return null;
//
//            String infix4 = getExampleByDkBricsAutomaton(new ArrayList<>(Collections.singleton(r4)), null, 0).getKey();
//
////        System.out.println("r0 = " + r0 + " r1 = " + r1 + " r2 = " + r2 + " r3 = " + r3 + " r4 = " + r4 + " counting = " + counting + " infix = " + infix1 + infix2 + infix3 + infix4);
//            return infix1 + infix2 + infix3 + infix4;
//        }
...
//    }
    ...
    private static Pair<String, SmtNode> judgeThirdCondition(String beta1, String beta2, String nodeCounting, int beta1Nullable, int beta2Nullable, boolean considerZeroWidthAssertions) throws InterruptedException {
//        if (beta1Nullable == 1 && beta2Nullable == 1) return "";
        if (!(beta1Nullable == 1 || beta2Nullable == 1)) return new Pair<>("", new SmtNode(Collections.singletonList("")));
        ...
        if (considerZeroWidthAssertions) {
            ...
            Pair<String, SmtNode> infix = getExampleByDkBricsAutomaton2(regexList, 0);
            return infix.getKey() != null ? infix : new Pair<>("", new SmtNode(Collections.singletonList("")));
        } else {
            ...
            Pair<String, SmtNode> infix = getExampleByDkBricsAutomaton(regexList1);
            return infix.getKey() != null ? infix : new Pair<>("", new SmtNode(Collections.singletonList("")));
        }
...
//        return infix != null ? infix : "";
    ...
    private static Pair<String, SmtNode> judgeFourthCondition(String beta1, String beta2, int beta1Nullable, int beta2Nullable, boolean considerZeroWidthAssertions) throws InterruptedException {
//        if (beta1Nullable == 1 || beta2Nullable == 1) return "";
        if (!(beta1Nullable != 1 && beta2Nullable != 1)) return new Pair<>("", new SmtNode(Collections.singletonList("")));
        ...
        if (considerZeroWidthAssertions) {
            ...
            regexList.addAll(beta1Trans);
            regexList.addAll(beta2Trans);
            Pair<String, SmtNode> infix = getExampleByDkBricsAutomaton2(regexList, 1);
            if (infix.getKey() != null) return infix;
            ...
            for (int i = 0; i < allPossibleChildren.size(); i++) {
                ...
                if (isGeneralizedCountingNode(candidateNode)) {
                    ...
                    Pair<String, SmtNode> infix1 = new Pair<>("", new SmtNode(Collections.singletonList("")));
                    if (!r0.equals("")) {
                        TreeNode r0Tree = createReDoSTree(r0);
                        getTranslateRegexForAssertionsListHelper(r0Tree);
                        List<String> r0Trans = getTranslateRegexForAssertionsList(r0Tree);
                        infix1 = getExampleByDkBricsAutomaton2(r0Trans, 1);
                        if (infix1.getKey() == null) continue;
                    }
                    String r4 = getR4(betaTree, candidateNode);            // 第二个counting后面的正则
                    Pair<String, SmtNode> infix2 = new Pair<>("", new SmtNode(Collections.singletonList("")));
                    if (!r4.equals("")) {
                        TreeNode r4Tree = createReDoSTree(r4);
                        getTranslateRegexForAssertionsListHelper(r4Tree);
                        List<String> r4Trans = getTranslateRegexForAssertionsList(r4Tree);
                        infix2 = getExampleByDkBricsAutomaton2(r4Trans, 1);
                        if (infix2.getKey() == null) continue;
                    }
                    return new Pair<String, SmtNode>(infix1.getKey() + infix2.getKey(),
                            new SmtNode(infix1.getValue().getIntersectionRegexes(),
                                    new SmtNode(infix2.getValue().getIntersectionRegexes())
                            )
                    );
...
//        System.out.println("infix = " + infix);
            ...
            for (int i = 0; i < allPossibleChildren.size(); i++) {
                ...
                if (isGeneralizedCountingNode(candidateNode)) {
                    ...
                    return new Pair<>(infix1.getKey() + infix2.getKey(),
                            new SmtNode(infix1.getValue().getIntersectionRegexes(),
                                    new SmtNode(infix2.getValue().getIntersectionRegexes())
                            )
                    );
        ...
        return new Pair<>("", new SmtNode(Collections.singletonList("")));
...
//                                        || (!Collections.disjoint(beta2LastSet, beta1FirstSet) && acceptEmptyString(rList.get(rList.size() - 1)))
                            ...
                            Pair<String, SmtNode> prefix1 = getPrefixString(standardizedReDoSTree, node, true); // 获取的是前缀正则+中缀正则的生成串
                            Pair<String, SmtNode> prefix2 = getPrefixString(standardizedReDoSTree, node, false);
                            String prefix3string = treeNodeCopy.getMatchStr(treeNodeCopy.getChildByChainIndexString(node.getChainIndex()));
                            Pair<String, SmtNode> prefix3 = new Pair<>(prefix3string, new SmtNode(Collections.singletonList(prefix3string)));

                            Set<Pair<String, SmtNode>> prefixSet = new HashSet<>();
                            if (prefix1.getKey() != null) prefixSet.add(prefix1);
                            if (prefix2.getKey() != null) prefixSet.add(prefix2);
                            if (prefix3.getKey() != null) prefixSet.add(prefix3);
...
//                            if (infixSet.isEmpty()) continue;
                            ...
                            Set<Pair<String, SmtNode>> infixSet1 = getInfixSetForEOA(r0, r1, r2, r3, r4, candidateCountingList, false);
                            Set<Pair<String, SmtNode>> infixSet2 = getInfixSetForEOA(r0, r1, r2, r3, r4, candidateCountingList, true);
                            if (infixSet1 != null) infixSet.addAll(infixSet1);
                            if (infixSet2 != null) infixSet.addAll(infixSet2);
                            if (infixSet.isEmpty()) continue;


                            Set<Pair<String, SmtNode>> suffixSet = new HashSet<>();
                            String treeNodeCopyNonMatchStr = treeNodeCopy.getNonMatchStr();
                            suffixSet.add(new Pair<String, SmtNode>(treeNodeCopyNonMatchStr, new SmtNode(Collections.singletonList(treeNodeCopyNonMatchStr))));
                            suffixSet.add(new Pair<String, SmtNode>(specialSuffixStringWithAllCountingNodes, new SmtNode(Collections.singletonList(specialSuffixStringWithAllCountingNodes))));
                            suffixSet.add(new Pair<String, SmtNode>(specialSuffixStringWithCountingAndEndLine, new SmtNode(Collections.singletonList(specialSuffixStringWithCountingAndEndLine))));
//                            suffixSet.addAll(specialSuffixStringWithNegativeAssertNode);
                            for (String theSpecialSuffixStringWithNegativeAssertNode: specialSuffixStringWithNegativeAssertNode) {
                                suffixSet.add(new Pair<String, SmtNode>(theSpecialSuffixStringWithNegativeAssertNode, new SmtNode(Collections.singletonList(theSpecialSuffixStringWithNegativeAssertNode))));
                            }
...
//                            if (specialSuffixStringWithCountingAndEndLine != null) suffixSet.add(specialSuffixStringWithCountingAndEndLine);
                            ...
                            for (Pair<String, SmtNode> infix: infixSet) {
                                suffixSet.addAll(getSuffixString(standardizedReDoSTree, node, infix.getKey()));
                            }
                            if (suffixSet.isEmpty()) continue;

                            for (Pair<String, SmtNode> suffix: suffixSet) {
                                for (Pair<String, SmtNode> prefix: prefixSet) {
                                    for (Pair<String, SmtNode> infix: infixSet) {
...
//                                        attackBean.setPatternType(PatternType.EOA_i_or_ii);
    ...
    //                        Collections.disjoint(beta1FirstSet, beta2FollowLastUnionLastSet)) continue;
                            ...
                            // β1可空或β2可空的情况
                            Set<Pair<String, SmtNode>> thirdConditionInfixSet = new HashSet<>();
                            Pair<String, SmtNode> thirdConditionInfix1 = judgeThirdCondition(beta1.toString(), beta2.toString(), nodeCounting, beta1Nullable, beta2Nullable,false);
                            Pair<String, SmtNode> thirdConditionInfix2 = judgeThirdCondition(beta1.toString(), beta2.toString(), nodeCounting, beta1Nullable, beta2Nullable,true);
                            if (!thirdConditionInfix1.getKey().equals("")) thirdConditionInfixSet.add(thirdConditionInfix1);
                            if (!thirdConditionInfix2.getKey().equals("")) thirdConditionInfixSet.add(thirdConditionInfix2);
                            if (!thirdConditionInfixSet.isEmpty()) {
                                Set<Pair<String, SmtNode>> suffixSet = new HashSet<>();
                                String treeNodeCopyNonMatchStr = treeNodeCopy.getNonMatchStr();
                                suffixSet.add(new Pair<>(treeNodeCopyNonMatchStr, new SmtNode(Collections.singletonList(treeNodeCopyNonMatchStr))));
                                suffixSet.add(new Pair<>(specialSuffixStringWithAllCountingNodes, new SmtNode(Collections.singletonList(specialSuffixStringWithAllCountingNodes))));
                                suffixSet.add(new Pair<>(specialSuffixStringWithCountingAndEndLine, new SmtNode(Collections.singletonList(specialSuffixStringWithCountingAndEndLine))));
//                                suffixSet.addAll(specialSuffixStringWithNegativeAssertNode);
                                for (String theSpecialSuffixStringWithNegativeAssertNode: specialSuffixStringWithNegativeAssertNode) {
                                    suffixSet.add(new Pair<>(theSpecialSuffixStringWithNegativeAssertNode, new SmtNode(Collections.singletonList(theSpecialSuffixStringWithNegativeAssertNode))));
                                }
//                                specialSuffixStringWithCountingAndEndLine = getSpecialSuffixStringWithCountingAndEndLine(specialParentR4Tree, true);
//                                if (specialSuffixStringWithCountingAndEndLine != null) suffixSet.add(specialSuffixStringWithCountingAndEndLine);
//                                specialSuffixStringWithCountingAndEndLine = getSpecialSuffixStringWithCountingAndEndLine(specialParentR4Tree, false);
//                                if (specialSuffixStringWithCountingAndEndLine != null) suffixSet.add(specialSuffixStringWithCountingAndEndLine);
                                ...
                                for (Pair<String, SmtNode> infix: thirdConditionInfixSet) {
                                    suffixSet.addAll(getSuffixString(standardizedReDoSTree, node, infix.getKey()));
                                }
                                if (suffixSet.isEmpty()) continue;
                                ...
                                for (Pair<String, SmtNode> suffix : suffixSet) {
                                    for (Pair<String, SmtNode> prefix : prefixSet) {
                                        for (Pair<String, SmtNode> thirdConditionInfix : thirdConditionInfixSet) {
                                            AttackBean attackBean = new AttackBean();
                                            attackBean.setPrefix(prefix);
...
//                                            attackBean.setPatternType(PatternType.EOA_iii);
                            ...
                            // β1不可空且β2不可空的情况
                            Set<Pair<String, SmtNode>> fourthConditionSet = new HashSet<>();
                            Pair<String, SmtNode> fourthConditionInfix1 = judgeFourthCondition(beta1.toString(), beta2.toString(), beta1Nullable, beta2Nullable, false);
                            Pair<String, SmtNode> fourthConditionInfix2 = judgeFourthCondition(beta1.toString(), beta2.toString(), beta1Nullable, beta2Nullable, true);
                            if (!fourthConditionInfix1.getKey().equals("")) fourthConditionSet.add(fourthConditionInfix1);
                            if (!fourthConditionInfix2.getKey().equals("")) fourthConditionSet.add(fourthConditionInfix2);
...
//                                if (specialSuffixStringWithCountingAndEndLine != null) suffixSet.add(specialSuffixStringWithCountingAndEndLine);
                                ...
                                for (Pair<String, SmtNode> suffix : suffixSet) {
                                    for (Pair<String, SmtNode> prefix : prefixSet) {
                                        for (Pair<String, SmtNode> fourthConditionInfix : fourthConditionSet) {
                                            AttackBean attackBean = new AttackBean();
                                            attackBean.setPrefix(prefix);
