...
//        deleteZeroWidthAssertion(regexTree);
    ...
    // 生成中缀串forEOD
    private static Pair<String, SmtNode> generateInfixStringForEOD(String regex1, String regex2, int model, String firstCharacter, boolean considerZeroWidthAssertions) throws InterruptedException {
            ...
            while (!stack.isEmpty()) {
                ...
                if (isGeneralizedCountingNodeWithMaxNumGreaterThanOne(node)) {
                    ...
                    for (TreeNode orNode : allOrNodeList) {
                        Set<Pair<String, SmtNode>> infix1Set = new HashSet<>();
                        Set<Pair<String, SmtNode>> infix3Set = new HashSet<>();
                        ...
                        if (infix1.getKey() != null) infix1Set.add(infix1);
                        Pair<String, SmtNode> infix3 = getExampleByDkBricsAutomaton2(r4Trans, 0);
                        if (infix3.getKey() != null) infix3Set.add(infix3);
...
//                                    regex2n = repairLineFeedsCarriageReturnsAndTabCharacters(regex2n);
                                    ...
                                    Pair<String, SmtNode> infix2 = null;
                                    infix2 = generateInfixStringForEOD(regex1n, regex2n, 1, firstCharacter, false);
                                    if (infix2.getKey() != null) infixSet.add(infix2);
...
//                                    suffixSet.addAll(specialSuffixStringWithNegativeAssertNode);
                                    for (String specialSuffixStringWithNegativeAssert : specialSuffixStringWithNegativeAssertNode) {
                                        suffixSet.add(new Pair<>(specialSuffixStringWithNegativeAssert, new SmtNode(Collections.singletonList(specialSuffixStringWithNegativeAssert))));
                                    }
...
//                                    if (specialSuffixStringWithCountingAndEndLine != null) suffixSet.add(specialSuffixStringWithCountingAndEndLine);
                                    ...
                                    Set<Pair<String, SmtNode>> fullInfixSet = new HashSet<>();
                                    for (Pair<String, SmtNode> infix1s : infix1Set) {
                                        for (Pair<String, SmtNode> infix : infixSet) {
                                            for (Pair<String, SmtNode> infix3s: infix3Set) {
                                                fullInfixSet.add(new Pair<>(infix1s.getKey() + infix.getKey() + infix3s.getKey(),
                                                        new SmtNode(infix1s.getValue().getIntersectionRegexes(),
                                                                new SmtNode(infix.getValue().getIntersectionRegexes(),
                                                                        new SmtNode(infix3s.getValue().getIntersectionRegexes())
                                                                )
                                                        )
                                                ));
                                            }
                                    ...
                                    for (Pair<String, SmtNode> fullInfix : fullInfixSet) {
                                        suffixSet.addAll(getSuffixString(standardizedReDoSTree, countingNode, fullInfix.getKey()));
                                    }
