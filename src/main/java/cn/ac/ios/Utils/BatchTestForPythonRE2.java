...
public class BatchTestForPythonRE2 {
    ...
    public static void generatePythonRE2File(String datasetName, String regex, ReDoSBean reDosBean, String attackFunctionName, String path) {
        ...
        String formatPrefix = attackBeanSuccess.getPrefix().getKey().replace("\\", "\\\\")
                .replace("\f", "\\f")
                .replace("\n", "\\n")
                .replace("\t", "\\t")
                .replace("\r", "\\r")
                .replace("\"", "\\\"")
                .replace("'", "\\'");
        String formatRepeat = attackBeanSuccess.getInfix().getKey().replace("\\", "\\\\")
                .replace("\f", "\\f")
                .replace("\n", "\\n")
                .replace("\t", "\\t")
                .replace("\r", "\\r")
                .replace("\"", "\\\"")
                .replace("'", "\\'");
        String formatSuffix = attackBeanSuccess.getSuffix().getKey().replace("\\", "\\\\")
                .replace("\f", "\\f")
                .replace("\n", "\\n")
                .replace("\t", "\\t")
                .replace("\r", "\\r")
